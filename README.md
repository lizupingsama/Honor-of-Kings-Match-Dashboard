...
| 玩家页 `/p/王者名称` | 段位 / 巅峰曲线、英雄统计、对局列表与详情；支持点赞（每浏览器每天一次） |
| 对局详情 `/matches/:id` | 单场对局明细 |
...
打开 `/leaderboard`，切换评分 / 排位 / 巅峰 / 英雄战力 / 胜率等维度；部分榜单可展开历史曲线。
...
当前仓库未附带正式开源许可证文件；若需二次分发，请先与维护者确认授权方式。
